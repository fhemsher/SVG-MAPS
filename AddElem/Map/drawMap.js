                ...
                if(group.nodeName!="#text")
                {
                    ...
                    if(mapId==id)
                    {
                            ...
                            if(email==CookieEmail)
                        {
                            ...
                            document.getElementById("inquiryYesCheck"+id).disabled = false
                        ...
                        for(var m = 0; m<MapIdArray.length; m++)
                        {
                            ...
                            if(myId!=mapId)
                            {
                                ...
                                if(editRow)
                                    ...
                                    var inquiryRow = document.getElementById("inquiryRow"+myId)
                                    inquiryRow.style.opacity = ".3"
...
function loadPreviewMap(id)
{
    ...
    xhr.onload = function()
    {
        ...
        //---DOMParser---
        var parser = new DOMParser();
        var previewDoc = parser.parseFromString(xmlString, "text/xml").documentElement;

        var groups = previewDoc.childNodes
...
function closePreviewMap(id)
{
    ...
    for(var k = domElemG.childNodes.length-1; k>=0; k--)
        ...
        getMapLibraryButton.innerHTML = "Custom Maps"
        enableAllButtons()
        if(ClearedMarkerId)
...
function hideMap(id)
{
    ...
    for(var k = domElemG.childNodes.length-1; k>=0; k--)
        ...
        document.getElementById("inquiryYesCheck"+id).disabled = true
        ...
        if(EditTitle)
    {
        ...
        document.getElementById("inquiryYesCheck"+id).style.background = EditTitleBG
...
function editMap(id)
{
    ...
    document.getElementById("inquiryYesCheck"+EditMapId).disabled = false
...
function finishEditMap()
{
    ...
    saveMap.setAttribute("title", title)
      ...
      var inquiryCheck=document.getElementById("inquiryYesCheck"+EditMapId)
       if(inquiryCheck.checked)
         saveMap.setAttribute("inquiry", "true")
       else
         saveMap.setAttribute("inquiry", "false")
       inquiryCheck.style.background=EditTitleBG
       inquiryCheck.disabled = true
...
function cancelEdit()
{
    ...
    document.getElementById("commentValue"+EditMapId).contentEditable = "false"
    document.getElementById("titleValue"+EditMapId).contentEditable = "false"
    document.getElementById("titleValue"+EditMapId).style.background = EditTitleBG
    document.getElementById("inquiryYesCheck"+EditMapId).disabled = true
    document.getElementById("commentValue"+EditMapId).style.background = EditTitleBG
    document.getElementById("inquiryYesCheck"+EditMapId).style.background = EditTitleBG

    openAddMapButton.disabled = true
    HideMapButton.disabled = false

    openAddMapButton.style.background = "#C3E6D3"
    getMapLibraryButton.innerHTML = "Showing Map..."
    BoundsRect.attr("stroke", "#004953")
    myMapTitleDiv.style.background = "#e0b0ff"
    getMapLibraryButton.style.background = "#C3E6D3"
